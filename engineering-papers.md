# Essential Engineering Papers for Distributed Systems and Big Data

As the tech industry continues to evolve, staying up-to-date with the latest advancements in distributed systems and big data is crucial. This curated list of engineering papers covers fundamental concepts and innovative solutions from industry leaders like Google, Amazon, Facebook, and more. Whether you're a seasoned engineer or just starting your journey, these papers offer invaluable insights into the architecture and design of large-scale systems.

## Distributed File Systems and Storage

- [Google File System (GFS)](https://research.google/pubs/the-google-file-system/)
- [Apache Hadoop Distributed File System (HDFS)](https://hadoop.apache.org/docs/stable/hadoop-project-dist/hadoop-hdfs/HdfsDesign.html)
- [Colossus: Successor to Google File System](https://www.infoq.com/news/2021/04/google-colossus/)
- [Megastore: Scalable Data System](https://www.cidrdb.org/cidr2011/Papers/CIDR11_Paper32.pdf)

## Big Data Processing

- [MapReduce: Simplified Data Processing on Large Clusters](https://static.googleusercontent.com/media/research.google.com/en//archive/mapreduce-osdi04.pdf)
- [Apache Hadoop: A Framework for Running Applications on Large Clusters](https://hadoop.apache.org/)
- [Apache Flink: Stream and Batch Processing in a Single Engine](https://flink.apache.org/)
- [Hive: A Warehousing Solution Over a Map-Reduce Framework](https://hive.apache.org/)

## Databases

### NoSQL
- [Bigtable: A Distributed Storage System for Structured Data](https://research.google/pubs/bigtable-a-distributed-storage-system-for-structured-data/)
- [Amazon DynamoDB: A Scalable, Predictably Performant, and Fully Managed NoSQL Database Service](https://aws.amazon.com/dynamodb/)
- [Cassandra: A Decentralized Structured Storage System](https://cassandra.apache.org/)

### NewSQL
- [Spanner: Google's Globally-Distributed Database](https://research.google/pubs/spanner-googles-globally-distributed-database/)
- [FoundationDB: A Distributed Unbundled Transactional Key Value Store](http://muratbuffalo.blogspot.com/2022/03/foundationdb-distributed-unbundled.html)

### Time Series Databases
- [Monarch: Google's Planet-Scale In-Memory Time Series Database](https://www.vldb.org/pvldb/vol13/p3181-adams.pdf)
- [Gorilla: A Fast, Scalable, In-Memory Time Series Database](https://www.vldb.org/pvldb/vol8/p1816-teller.pdf)

### Graph Databases
- [TAO: Facebook's Distributed Data Store for the Social Graph](https://www.youtube.com/watch?v=sNIvHttFjdI)

### Data Warehousing
- [Napa: Powering Scalable Data Warehousing with Robust Query Performance at Google](https://research.google/pubs/napa-powering-scalable-data-warehousing-with-robust-query-performance-at-google/)
- [Mesa: Geo-Replicated, Near Real-Time, Scalable Data Warehousing](https://research.google/pubs/mesa-geo-replicated-near-real-time-scalable-data-warehousing/)

## Distributed Systems

- [The Chubby Lock Service for Loosely-Coupled Distributed Systems](https://www.recurse.com/blog/49-paper-of-the-week-the-chubby-lock-service-for-loosely-coupled-distributed-systems)
- [Large-scale cluster management at Google with Borg](https://research.google/pubs/large-scale-cluster-management-at-google-with-borg/)
- [Twine: A Unified Cluster Management System for Shared Infrastructure](https://www.usenix.org/conference/osdi20/presentation/tang)

## Authentication and Security

- [Zanzibar: Google's Consistent, Global Authorization System](https://research.google/pubs/zanzibar-googles-consistent-global-authorization-system/)

## Tracing and Monitoring

- [Dapper, a Large-Scale Distributed Systems Tracing Infrastructure](https://research.google/pubs/dapper-a-large-scale-distributed-systems-tracing-infrastructure/)

## Graph Processing

- [Pregel: A System for Large-Scale Graph Processing](https://15799.courses.cs.cmu.edu/fall2013/static/papers/p135-malewicz.pdf)

## Machine Learning and AI

- [TensorFlow: A System for Large-Scale Machine Learning](https://www.usenix.org/conference/osdi16/technical-sessions/presentation/abadi)
- [Prophet: Forecasting at Scale](https://research.facebook.com/blog/2017/2/prophet-forecasting-at-scale/)

## Analytics

- [F1: A Distributed SQL Database That Scales](https://research.google/pubs/f1-a-distributed-sql-database-that-scales/)

## Content Delivery

- [HALP: High-Availability and Low-Latency Paxos](https://www.usenix.org/conference/nsdi23/presentation/li-mingyu)

## Cloud Services

- [Google Cloud Firestore](https://cloud.google.com/firestore)
- [Amazon Aurora: Design Considerations for High Throughput Cloud-Native Relational Databases](https://www.amazon.science/publications/amazon-aurora-design-considerations-for-high-throughput-cloud-native-relational-databases)

## Real-time Processing

- [Monolith: Real Time Recommendation System With Collisionless Embedding Table](https://arxiv.org/abs/2209.07663)

## Caching

- [Scaling Memcache at Facebook](https://www.usenix.org/conference/nsdi13/technical-sessions/presentation/nishtala)

## Consensus Algorithms

- [FlexiRaft: Flexible Quorums with Raft](https://www.usenix.org/conference/atc23/presentation/kowalski)

## Network Analysis

- [MilliSampler: Fine-Grained Network-Wide Packet Sampling](https://arxiv.org/abs/2001.05074)

## Root Cause Analysis

- [MineSweeper: An In-depth Look Into Ransomware Infection Chain](https://www.usenix.org/conference/usenixsecurity20/presentation/kharaz)

## Service Management

- [ServiceRouter: A Service Mesh for Collaborating Microservices](https://research.facebook.com/publications/servicerouter-a-service-mesh-for-collaborating-microservices/)

## Messaging and Event Streaming

- [Apache Kafka: A Distributed Streaming Platform](https://kafka.apache.org/)

## Interoperability

- [Apache Thrift: A Language-Independent Interface Definition Language](https://thrift.apache.org/)

## Performance and Scalability

- [Scalability! But at what COST?](https://www.usenix.org/conference/hotos15/workshop-program/presentation/mcsherry)

These papers represent the foundation of modern distributed systems and big data technologies. They offer deep insights into the challenges and solutions in building scalable, reliable, and efficient systems. Whether you're looking to understand the principles behind popular technologies or seeking inspiration for your next project, these resources are invaluable.

Happy reading, and may these papers inspire your next big idea or solution!
